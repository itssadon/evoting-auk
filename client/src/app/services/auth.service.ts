...
export class AuthService {
  ...
  registerUser(user) {
    ...
    return this.http
      .post(this.apiService.getAPI() + 'users/register', user, {
        headers: headers
      })
      .map(res => res.json());
  }
  ...
  authenticateUser(user) {
    ...
    headers.append('Content-Type', 'application/json');
    return this.http
      .post(this.apiService.getAPI() + 'users/authenticate', user, {
        headers: headers
      })
      .map(res => res.json());
  }
  ...
  getProfile() {
    ...
    headers.append('Content-Type', 'application/json');
    return this.http
      .get(this.apiService.getAPI() + 'users/profile', { headers: headers })
      .map(res => res.json());
  }
  ...
  canPerformTask(task) {
    ...
    if (task === 'password' && (user_role === 0 || user_role === 1)) {return true; }
  ...
  resetUserPassword(user) {
    ...
    headers.append('Content-Type', 'application/json');
    return this.http
        .post(this.apiService.getAPI() + 'users/password', user, {
            headers: headers
        })
        .map(res => res.json());
  }
