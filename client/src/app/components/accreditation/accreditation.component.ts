...
export class AccreditationComponent implements OnInit {
    ...
    onAccreditSubmit() {
        var pageDimmer = '<div class="ui inverted page dimmer active" id="pageDimmer"><div class="content"><div class="center"><div class="center"><div class="ui indeterminate large text loader"><h3>Accrediting student.<br><br>Please wait...</h3></div></div></div></div></div>';
                    ...
                    this.authService.registerUser(user).subscribe(
                        data => {
                            if(data.success) {
                                ...
                                this.toasterService.pop('error', 'Oops!', 'Something went wrong adding student database');
                            }
                            this.ngOnInit();
                            $('#pageDimmer').remove();
                        },
    ...
    testSendSMS() {
        ...
        this.smsService.sendSMS(smsDetails).subscribe(
            response => {
                this.toasterService.pop('success', 'Success', 'SMS Sent');
            },
            error => {
                this.toasterService.pop('error', 'Oops!', error);
            }
